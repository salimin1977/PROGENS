...
export class SupabaseDataProvider implements DataProvider {
  async getStudents(): Promise<Student[]> {
    ...
    return (studentQuery.data ?? []).map((row) => {
      ...
      const academicScore = hasAssessment ? Math.round((validResults.reduce((sum, result) => sum + (Number(result.marks ?? 0) / Number(result.max_marks)) * 100, 0) / validResults.length) * 10) / 10 : 0;
  ...
  async getAcademicResults(): Promise<AcademicResult[]> {
    ...
    if (error) throw error; if (subjectError) throw subjectError; if (assessmentError) throw assessmentError;
    const subjectMap = new Map((subjects ?? []).map((row) => [row.id, row.name]));
    const assessmentMap = new Map((assessments ?? []).map((row) => [row.id, row]));
    return (data ?? []).map((row) => {
      const assessment = assessmentMap.get(row.assessment_id);
      return { id: row.id, studentId: row.student_id, subject: subjectMap.get(row.subject_id) ?? row.subject_id, assessment: (assessment?.assessment_type ?? 'PPT') as AcademicResult['assessment'], assessmentId: row.assessment_id, assessmentName: assessment?.name ?? '', assessmentDate: assessment?.assessment_date ?? null, marks: Number(row.marks ?? 0), maximumMarks: Number(row.max_marks ?? 100), grade: row.grade ?? '' };
    });
  }
  async getAttendance(): Promise<AttendanceRecord[]> {
    ...
    return (data ?? []).map((row) => ({ id: row.id, studentId: row.student_id, date: row.updated_at?.slice(0, 10) ?? '', absentDays: Number(row.absent_days ?? 0), academicYear: Number(row.academic_year ?? 2026), sourceLabel: row.source_label ?? 'Attendance summary' }));
  }
