...
export function buildCommandHQ(students: Student[], interventions: Intervention[]): CommandHQ {
  ...
  const signals: CommandSignal[] = [
    ...
    { title: 'Attendance signal', detail: absent10 === 0 ? 'Attendance summary is not embedded in the Student provider; use Data Health/Attendance for verified absent-day counts.' : `${absent10} student(s) have 10+ absent days.`, severity: 'WATCH' },
  ];
  const priorities = [
    critical > 0 ? 'Resolve Critical risk cases first.' : 'No Critical risk case detected in the current provider view.',
    high > 0 ? 'Review High-risk students and assign a next action.' : 'No High-risk case detected in the current provider view.',
    activeInterventions > 0 ? 'Close the intervention loop: action → outcome → next review.' : 'Maintain monitoring and document outcomes.',
  ];
