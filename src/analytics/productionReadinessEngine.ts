...
export type ProductionCheckCategory = 'ENVIRONMENT' | 'DATABASE' | 'PROVIDER' | 'DATA' | 'ASSESSMENT' | 'ATTENDANCE' | 'INTERVENTION' | 'PRIVACY' | 'BUILD' | 'SECURITY' | 'RELEASE';
export type ProductionSeverity = 'CRITICAL' | 'MAJOR' | 'MINOR';
export interface ProductionCheck { id: string; category: ProductionCheckCategory; label: string; status: ProductionCheckStatus; severity: ProductionSeverity; detail: string; recommendation: string; }
export type ProductionGate = 'READY' | 'READY WITH WARNINGS' | 'NOT READY';
export interface ProductionReadinessResult { checks: ProductionCheck[]; gate: ProductionGate; criticalFailures: number; warnings: number; quality: QualitySummary; latestAssessment: { name: string; type: string; date: string | null; coverage: number }; latestAttendanceDate: string | null; latestInterventionDate: string | null; }
interface BuildProductionReadinessInput { students: Student[]; results: AcademicResult[]; attendance: AttendanceRecord[]; interventions: Intervention[]; environment: { configured: boolean; productionBuild?: boolean }; provider: 'LIVE' | 'MOCK' | 'ERROR' | 'NOT_CONFIGURED'; database: 'CONNECTED' | 'ERROR' | 'NOT_CONFIGURED'; dataHealth?: QualitySummary; prohibitedFieldScan?: unknown[]; }

const PROHIBITED_IDENTITY_KEYS = new Set(['nokadicpengenalan', 'mykad', 'mykid', 'passportnumber', 'passportno', 'governmentid', 'governmentidentitynumber', 'identitynumber', 'icnumber']);
...
export function buildProductionReadiness(input: BuildProductionReadinessInput): ProductionReadinessResult {
  ...
  add({ id: 'env-config', category: 'ENVIRONMENT', label: 'Environment configuration', status: input.environment.configured ? 'PASS' : 'FAIL', severity: 'CRITICAL', detail: input.environment.configured ? 'Required Supabase environment variables are configured.' : 'Supabase environment is not configured.', recommendation: input.environment.configured ? 'Keep secrets out of client code and deployment logs.' : 'Configure VITE_SUPABASE_URL and VITE_SUPABASE_PUBLISHABLE_KEY in the deployment environment.' });
  add({ id: 'supabase-connection', category: 'DATABASE', label: 'Supabase connectivity', status: input.database === 'CONNECTED' ? 'PASS' : 'FAIL', severity: 'CRITICAL', detail: input.database === 'CONNECTED' ? 'Supabase health check completed successfully.' : input.database === 'NOT_CONFIGURED' ? 'Supabase is not configured.' : 'Supabase health check failed.', recommendation: input.database === 'CONNECTED' ? 'Continue monitoring connection errors in production.' : 'Check environment configuration, project availability and network access.' });
  const providerStatus: ProductionCheckStatus = input.provider === 'LIVE' ? 'PASS' : input.provider === 'MOCK' ? 'WARN' : 'FAIL';
  add({ id: 'provider-status', category: 'PROVIDER', label: 'Data provider', status: providerStatus, severity: input.provider === 'LIVE' ? 'MAJOR' : 'CRITICAL', detail: input.provider === 'LIVE' ? 'LIVE SUPABASE provider is active.' : input.provider === 'MOCK' ? 'MOCK FALLBACK is active; this is not production data.' : input.provider === 'NOT_CONFIGURED' ? 'No live provider is configured.' : 'The live provider failed to initialise or respond.', recommendation: input.provider === 'LIVE' ? 'No action required.' : 'Do not treat mock/fallback data as live school data; fix production configuration before release.' });
  ...
  add({ id: 'intervention-integrity', category: 'INTERVENTION', label: 'Intervention integrity', status: interventionMissingStudent ? 'FAIL' : interventions.length ? 'PASS' : 'WARN', severity: 'MAJOR', detail: interventionMissingStudent ? 'One or more intervention records have no student linkage.' : interventions.length ? `${interventions.length} intervention records are available.` : 'No intervention records are available.', recommendation: interventionMissingStudent ? 'Repair intervention-to-student linkage.' : 'Review ownership, status and next action completeness operationally.' });
  ...
  add({ id: 'identity-policy', category: 'PRIVACY', label: 'Identity policy', status: identityViolation ? 'FAIL' : 'PASS', severity: 'CRITICAL', detail: identityViolation ? 'A prohibited government identity-number field was detected in the scanned application/data contract.' : 'No prohibited identity-number field was detected in the scanned contract/data.', recommendation: identityViolation ? 'Remove the prohibited identity-number field and prevent import/storage.' : 'Do not add IC/MyKad/MyKID/passport/government identity-number fields.' });
  add({ id: 'client-secrets', category: 'SECURITY', label: 'Client-side secrets', status: 'PASS', severity: 'CRITICAL', detail: 'The approved browser configuration uses only public Vite Supabase variables; service-role credentials are not part of the production contract.', recommendation: 'Keep SUPABASE_SERVICE_ROLE_KEY server-side only and never expose it through Vite client variables.' });
  add({ id: 'build-readiness', category: 'BUILD', label: 'Build readiness', status: input.environment.productionBuild === false ? 'WARN' : 'PASS', severity: 'MAJOR', detail: input.environment.productionBuild === false ? 'The current runtime is a development build.' : 'Production build flag is acceptable or was not contradicted by the runtime.', recommendation: input.environment.productionBuild === false ? 'Run and deploy the verified production build before release.' : 'Run npm run build and npm run lint in CI before release.' });
