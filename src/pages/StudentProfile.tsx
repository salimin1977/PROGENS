...
export default function StudentProfile() {
  ...
  if (!student) {
    return (
      <EmptyState icon={FileQuestion} title="Student not found" description="This student record does not exist in the current dataset." />
    );
  }
      ...
      <div className="card flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">{student.id} &middot; {student.className} &middot; {student.form}</p>
        ...
        <div className="grid grid-cols-2 gap-3 sm:flex sm:gap-6">
          ...
          <Metric label="STEM Readiness" value={`${student.stemReadiness}`} />
        </div>
      </div>
      ...
      {tab === 'Profile' && (
        <div className="card p-5">
          ...
          <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Field label="Guardian Contact" value={student.guardianContact} icon={<Phone size={13} />} />
              ...
              {studentInterventions.map((i) => (
                <div key={i.id} className="rounded-lg border border-slate-200 p-3">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-semibold text-navy-950">{i.interventionType}</p>
...
function Field({ label, value, icon }: { label: string; value: string; icon?: React.ReactNode }) {
  return (
    <div>
      ...
      <dd className="mt-0.5 flex items-center gap-1 text-sm font-medium text-navy-900">
        {icon}
        {value}
      </dd>
    </div>
